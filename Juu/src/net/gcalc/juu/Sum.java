...
class Sum extends Folder<GNumber>
{
	Sum(Node node) {
		super(node);
		assert(node.getId()==JJTSUM);
	}
	
	protected Environment start(Deque<Call> stack, Environment env) {
		value = new GInteger(0);
		return env;
	}
	
	protected GNumber fold(Object data) {
		if (currentChild.getId()==JJTMINUS) {					
			return value.subtract((GNumber) data);
		}
		return value.add((GNumber) data);
	}	
}