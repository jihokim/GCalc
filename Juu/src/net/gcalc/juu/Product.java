...
class Product extends Folder<GNumber> 
{
	Product(Node node) {
		super(node);
		assert(node.getId()==JJTPROD);
	}
	
	protected Environment start(Deque<Call> stack, Environment env) {
		value = new GInteger(1);
		return env;
	}
	
	protected GNumber fold(Object data) {
		if (currentChild.getId()==JJTDIV) {					
			return value.divide((GNumber) data);
		}
		return value.multiply((GNumber) data);
	}	
}